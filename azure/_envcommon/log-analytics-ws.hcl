...
#   resource_tags         = { "arch" = "log-analytics:${local.module_version}", "bmw_region" = "${local.region_vars.locals.bmw_region}", "env" = "${local.env_vars.locals.env}" }
...
#   log_analytics_ws_name = replace("${local.global_vars.locals.resource_names.log_analytics_ws_name}", "$PLACEHOLDER", "${local.region_vars.locals.bmw_region}-${local.env_vars.locals.env}")
# }
...
dependency "rg" {
  ...
  mock_outputs = {
    rg_name = "MOCK-aiqx-app-logging-rg"
  }
}
